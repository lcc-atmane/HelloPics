package com.phantomhive.exil.hellopics.Img_Editor.Views.Cropper;
...
import com.phantomhive.exil.hellopics.Img_Editor.Views.Cropper.Util.HandleUtil;
import com.phantomhive.exil.hellopics.Img_Editor.Views.Cropper.Util.PaintUtil;
...
public class
CropImageView extends ImageView {
    ...
    public Bitmap getCroppedImage() {
        ...
        PointF cropPointF  = xAndyFixer.FixXYUsingMatrix(Edge.LEFT.getCoordinate(),Edge.TOP.getCoordinate(),getImageMatrix(),0);


        // Calculate the top-left corner of the crop window relative to the ~original~ bitmap size.
        final float cropX =cropPointF.x;
        final float cropY = cropPointF.y;
        ...
        xAndyFixer.FixRect(Edge.LEFT.getCoordinate(),Edge.TOP.getCoordinate(),
                Edge.RIGHT.getCoordinate(),Edge.BOTTOM.getCoordinate(),
                getImageMatrix());


        final float cropWidth = Math.min(xAndyFixer.getFixedRectF().width() , originalBitmap.getWidth() - cropX);
        final float cropHeight = Math.min(xAndyFixer.getFixedRectF().height() , originalBitmap.getHeight() - cropY);
